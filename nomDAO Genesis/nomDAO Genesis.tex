...
\title{nomDAO hop-paper \\ {\it into the nom\/} } 
...
\\
\normalsize{$^{1}$Department of Anomalous Bunnynomics, University of nomDAO,}\\
\normalsize{SolBunny Street, nomTown, SBM 1337, Noom}\\
\\
\normalsize{$^{2}$Supreme leaders at The University of nomDAO}\\
\\
\normalsize{$^\ast$To whom correspondence should be addressed; E-mail: Bunnynomics101@gmail.com}\\
\normalsize{ \raisebox{-.9\mydepth}{\includegraphics[height=+1.2\myheight]{nom}} \href{https://www.nomdao.com}{nomDAO} $\vert$ \faIcon{github} \href{https://github.com/BunnyNomics101}{Github} $\vert$ \faIcon{twitter} \href{https://twitter.com/TheNomDAO}{Twitter} $\vert$ \faIcon{discord} \href{https://discord.gg/uWCfmp6daq} {Discord} 
$\vert$ \faIcon{reddit} \href{https://www.reddit.com/r/nomDAO/}{Reddit}}
\\
\date{\today}
}
...
\begin{sciabstract}
  This hop-paper serves as a whitepaper for {\it nomDAO\/} (nom token), and describe the vision of its ecosystem. 
  nomDAO is presented as a new form of DAO based on liquid democracy with the capacity to run a decentralized exchange with launchpad, a bioreactor—something between a hedge fund and a business incubator. In addition, the protocol remains completely decentralized and autonomous despite a high complexity of connected applications. 
 The new era of DAOs will allow its users more control over their financial situation, both on-chain and in real-word situations.
\end{sciabstract}
...
nomDAO invests its treasury according to the 80/20-rule (Pareto principle) with 80$\%$ off-chain and 20 $\%$ on-chain in microfinance or liquid assets, respectively. The allocation is not fixed and will be re-adjusted as needed to achieve the optimal portfolio composition. In certain cases the treasury will hold illiquid assets in its treasury although mainly for collaboration purposes with other projects on-chain while the majority is in USDC or other similar stablecoins. Off-chain strategies are focused on providing financial needs to startups with no access to regular bank loans and thus, the risk tolerance is higher than traditional investment firms (venture capital, private equity, etc.). In addition, nomDAO will provide adequate tools for them to onboard web3 so the dual function of nomDAO as both a hedge fund and a business incubator make it a 'bioreactor'.

tl;dr Participate in the nomDAO ecosystem.

\clearpage

Figure 2: nomDAOs hybrid-DeFi \\
...
\node[squarednode]      (leftsquare2)       [below=of leftsquare] {nom};
